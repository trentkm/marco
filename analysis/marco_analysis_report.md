...
## Tissue-Resident Reprogramming vs Monocyte Infiltration

A key question is whether the pro-inflammatory MARCO+ cells seen during colitis represent (a) anti-inflammatory tissue-resident macrophages that have transcriptionally reprogrammed, or (b) newly infiltrating monocyte-derived cells expressing MARCO. The data shows **it's both, but the dominant signal depends on tissue context**.
...
### Evidence for tissue-resident MARCO+ macrophages in the muscularis
...
The baseline Marco+ cells in the muscularis **are** tissue-resident (Timd4+/Mertk+), but they already have a pro-inflammatory lean at steady state — they are not classically anti-inflammatory like the Lyve1+/CD163+ population. During colitis, new monocytes flood in and also express Marco, diluting the resident signal.

### The Gut Atlas confirms two coexisting populations
...
- This confirms two distinct MARCO+ populations coexist in the normal colon: incoming monocytes and fully differentiated residents
...
The tissue-resident Marco+ macrophages in the muscularis (Timd4+/Mertk+) are **not classically anti-inflammatory to begin with** — they are already Il6-hi/Tnf-hi/Lyve1-lo at baseline, distinct from the anti-inflammatory Lyve1+/CD163+ population. Therefore:

1. There is a **constitutive pro-inflammatory-leaning resident** Marco+ population (Cx3cr1+/Timd4+) that persists through colitis
...
**Open question**: Whether individual resident Marco+ cells are also transcriptionally shifting during inflammation, or whether the observed changes are entirely compositional (monocyte influx), cannot be fully resolved without lineage tracing combined with a Marco reporter. Stavely et al. confirmed that resident macrophages persist during colitis, but did not track Marco expression specifically.
